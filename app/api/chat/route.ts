...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    const sys = [
      ...
      "If 'Curated spots' context is given, ground venue-specific facts (area, vibe, monsoon suitability, cost) in that data — never invent an address, price, or opening hours for a place listed there. If the user states something about a venue (its location, area, or any other fact) that conflicts with the data you were given, correct them plainly — never just go along with an incorrect premise to be agreeable.",
