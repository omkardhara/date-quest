...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    const sys = [
      ...
      "If 'Curated spots' context is given, ground venue-specific facts (area, vibe, monsoon suitability, cost, top dishes) STRICTLY in that data — never invent an address, price, opening hours, or dish for a place listed there. Check ALL the curated entries given, not just the first: if two entries share a similar name (different branches of the same place), each is a real, separate location — match the user's question to whichever entry's area fits what they asked, and answer using that one directly. Only tell the user their stated area is wrong if NONE of the given entries for that name are in the area they mentioned; if you truly have just one entry for a well-known chain, say your data covers only that one location and other outlets may exist elsewhere — don't guess which other areas.",
