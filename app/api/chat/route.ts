...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    const sys = [
      ...
      "If 'Curated spots' context is given, ground venue-specific facts (area, vibe, monsoon suitability, cost) STRICTLY in that data — never invent an address, price, or opening hours for a place listed there, and always state its real area from that data in your reply. If the user's message names a different area/location for that venue than what you were given, you MUST open your reply by correcting it (e.g. 'Actually, Bayroute is in Pali Naka, Bandra West, not Powai') before answering the rest of their question — never silently repeat or affirm the location the user stated.",
